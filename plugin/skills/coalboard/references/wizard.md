...
3. **ONE plain bill = the only question:** "I'll have 3 independent reviewers + a judge check [target] ([N] files) — about [Y] tokens. [Go ✓ · cheaper · more thorough · cancel]". NO opaque jargon (cheaper/more-thorough map to depth/rigor silently). Picking `cheaper`/`more thorough` = a CHANGE → RECOMPUTE the bill + re-ask (a fresh bill box); only `Go` on the SHOWN bill convenes.
...
- **Bill → Call 3 — confirm:** compute the cost FROM the picks → show the bill (target · N files · resolved config · model-per-lens · ~cost) → ONE question: confirm ✓ · change · cancel. **On `change`: loop back to Call 2 (re-pick) → RECOMPUTE the bill → re-present THIS confirm as a FRESH box** — a change can multiply cost (e.g. → nasa), so NEVER spawn on a stale bill and NEVER fold the recompute into the change step. The loop is `order → bill → change → bill → pay`. Spawn the first lens ONLY on a confirm of the CURRENT bill. DISPATCH defaults all-at-once (config-overridable; ask only if raised). Boarding-scope per `audit.md`.
