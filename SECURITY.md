...
Last scan: **NVIDIA SkillSpector v2.2.3** (self-reported — the tool ships no tagged releases, so the version is the `uvx`-from-git HEAD, pinned by commit not a release tag), **2026-06-19**, on the shipped `plugin/` dist (skill + hook + command files). Re-scanning is **event-driven on a NEW SkillSpector version (maintainer-commanded), NOT per CoalBoard release** — SkillSpector's static analyzers are stable (unchanged since its 2026-05-11 release), so a CoalBoard version bump that changes skill/hook/lib content does NOT alter what these (non-MCP) static rules read. **This v1.0.1 scan + its all-false-positive verdict therefore HOLD across later versions, and stand until SkillSpector itself ships new rules.**
