...
- **Local files only.** All state lives in files you can read: the config (`~/.claude/.coalboard.json` and an optional per-project `.claude/.coalboard.json`) and the project-scoped `.coalboard/` staging directory (`proposed/` = staged changes awaiting your approval · `reports/` = audit reports & post-mortems · `.gitignore` = the self-fence the board plants before its first file, keeping the whole dir out of the host repo's commits).
