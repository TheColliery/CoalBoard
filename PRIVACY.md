...
- **Local files only.** All state lives in files you can read: the config (`~/.claude/.coalboard.json` and an optional per-project `.claude/.coalboard.json`) and the project-scoped `.coalboard/` staging directory.
