...
- **Propose, never execute.** The board writes proposals to a local `.coalboard/proposed/` staging directory; nothing touches your live files or fires a side-effect until you approve. Credential patterns are scrubbed from anything logged or displayed.
...
- **Local files only.** All state lives in files you can read: the config (`~/.claude/.coalboard.json` and an optional per-project `.coalboard.json`) and the project-scoped `.coalboard/` staging directory.
