...
## Model assignment (DETERMINISTIC — R2-4 · an ORCHESTRATION param, NOT a prompt placeholder)
Per SKILL.md Step 1: default cost-bonus = cheap lenses (haiku) + opus judge; `lensTiers` pins override; `diversifyModels` (nasa ONLY) spreads across model GENERATIONS (different training runs), **NEVER across tiers** (haiku/sonnet/opus = one Claude family = shared blind spots, no decorrelation + over-cost). The assignment is IDENTICAL across every unit of a multi-unit run (deterministic, never arbitrary per-unit). Lead each spawn's label with the model. NEVER assign an access-gated / unavailable model (e.g. Fable) — drop it from the pool, re-route on a dead spawn.
