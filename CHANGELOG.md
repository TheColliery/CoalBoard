...
### Security
- **The project `.coalboard.json` layer could ESCALATE a consent-bearing key past an explicit global choice** (`coalboardMode`/`updateMode`: `off`/`ask` → `auto`) on TWO read paths — the conductor hook's own `readCfg()`, and `SKILL.md`'s own "project wins per key" merge instruction, which the AGENT follows to decide ask-vs-auto at Step 0 (the real consent decision, reachable via 3 of 4 activation paths — manual `/coalboard`, the agent's own judgment, a CoalTipple hand-off — that never touch the hook). **Round 1 (`3f68bb5`) fixed only the hook's `readCfg()`/`mergeSafety()`, and its CHANGELOG line overclaimed full closure — corrected here.** This round closes the SKILL.md half (a clamp-aware merge instruction, same safer-value-wins rule) and a second hook-side gap: `mergeSafety()` skipped the clamp entirely when global had no explicit value, leaving an unconfigured user (the common case) unprotected — an absent global now anchors on the schema default (`ask`) instead of "anything goes." `fableConsent`/`applyConsent` remain correctly out of reach (the hook never reads either key at all) — unchanged from round 1, still a named gap, not a regression. Mirrors CoalMine `updateMode` (v3.9.3) / CoalWash `mergeSafety` for the hook shape; the SKILL.md clamp and the absent-global fix are new here, not yet ported to those two exemplars (both carry the same absent-global gap per this round's review). Case-fold was correct but untested — a mixed-case regression fixture added.

### Notes
- **CB-specific honesty note on the absent-global fix:** it has NO hook-observable effect for this repo — `boardOff()`/`updateDue()` both branch only on `off` vs non-off, so a value clamped to `ask` instead of left at an escalated `auto` cannot be told apart by a hermetic hook-spawn test. Shipped for `mergeSafety()`'s own correctness (and because SKILL.md's prose fix makes the SAME distinction observable to the agent reading it) — not cited as closing a hook-level escalation for CB.
