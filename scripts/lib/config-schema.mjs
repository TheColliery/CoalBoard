...
export const CONFIG_SCHEMA = [
  ...
  { key: 'callFable', type: 'bool', flags: ['--fable'], help: 'Feature-gate Fable on the board — Fable bills real usage credits outside plan limits, keep off unless you mean to spend. false (default): Fable is NEVER seated on any lens, and any lensTiers/rigorLensTiers entry naming it is inert (this gate OVERRIDES pins — it guards real money, not just cost). true: Fable MAY be seated on the highest-value seats only — the sub4 observer (episodic; fires only on deadlock/max-stakes) and optionally the truth/formal lens at rigor:nasa — never the security-focused lens (the platform safeguard reroutes Fable off security content). Governs LENS spawns only — never the judge, which always runs on main (whatever model the user is already running).' },
